...
//AVISOS DE MENSAJE
const smsAvisoRG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoAG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoIIG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoFG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoMG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoEEG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }
const smsAvisoEG = () => { return `*[🛑 ❱❱ *𝙸𝙽𝙵𝙾𝚁𝙼𝙰𝚃𝙸𝚅𝙾* ❰❰ 🛑]*\n\n` }

//PARÁMETROS EN COMANDOS
const smsRowner = () => { return `*Este comando solo puede ser realizado por los creadores del bot*` }//NUMERO DE BOT
const smsOwner = () => { return `*Este comando solo puede ser realizado por los creadores del bot*` }//OWNER
const smsMods = () => { return `¡¡𝙀𝙨𝙩𝙚 𝙘𝙤𝙢𝙖𝙣𝙙𝙤 𝙨𝙤𝙡𝙤 𝙥𝙪𝙚𝙙𝙚 𝙨𝙚𝙧 𝙪𝙩𝙞𝙡𝙞𝙯𝙖𝙙𝙤 𝙥𝙤𝙧 𝙢𝙤𝙙𝙚𝙧𝙖𝙙𝙤𝙧𝙚𝙨` }//MODERADORES
const smsPremium = () => { return `*Este comando solo puede ser usado por usuarios premium*` }//USUARIOS PREMIUM
const smsGroup = () => { return `*Este Comando solo se usar en grupos` }//PARA GRUPOS
const smsPrivate = () => { return `*Este comando solo puede ser usado al privado del bot*` }//AL PRIVADO
const smsAdmin = () => { return `*Este comando solo esta apto para los administradores del grupo*` }//ADMINS
const smsBotAdmin = () => { return `*Para realizar este comando es necesario que el bot sea admin*` }//BOT CON ADMIN
const smsUnreg = () => { return `*Hey alto no estas registrado*\n\n*Para poder usar el bot*\n\n*es necesario que estes en mi base de datos*\n\nEjemplo : .reg1 Dylan.18` }//VERIFICAR
const smsRestrict = () => { return `Comando desactivado por uno de mis creadores` }//RESTRINGIR COMANDO

//MENU LISTA
const smsTime = () => { return `𝙏𝙞𝙚𝙢𝙥𝙤 𝘼𝙘𝙩𝙪𝙖𝙡`}
const smsUptime = () => { return `𝙏𝙞𝙚𝙢𝙥𝙤 𝙖𝙘𝙩𝙞𝙫𝙤`}
const smsVersion = () => { return `𝙑𝙚𝙧𝙨𝙞𝙤𝙣 𝙙𝙚𝙡 𝘿𝙮𝙡𝙖𝙣𝘽𝙤𝙩-𝙈𝘿`}
...
//main.js
const smsWelcome = () => { return `*╭┈⊰* @subject *⊰┈ ✦*\n*│✨ 𝘽𝙄𝙀𝙉𝙑𝙀𝙉𝙄𝘿𝙊 (𝘼)!!*\n┊💖 @user\n│📚 𝙇𝙀𝘼 𝙇𝘼 𝘿𝙀𝙎𝘾𝙍𝙄𝙋𝘾𝙄𝙊𝙉 𝘿𝙀𝙇 𝙂𝙍𝙐𝙋𝙊\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈ ✦*\n${String.fromCharCode(8206).repeat(850)}\n@desc`}
const smsBye = () => { return '*╭┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*\n│ @user\n│ *QUE TE VALLA MAL , CHAO!!* 😎\n*╰┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈┈⊰*'}
const smsSpromote = () => { return '*[🛑] @user es un nuevo admin del grupo 🙈*'}
const smsSdemote = () => { return '*[🛑] dejo de ser admin que triste :((**'}
...
//_allantilink.js
const smsTextoYT = () => { return '🤩 𝙎𝙪𝙥𝙚𝙧 𝘿𝙮𝙡𝙖𝙣𝘽𝙤𝙩-𝙈𝘿 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 '} 
...
const smsInt2 = () => { return `En este grupo no estan permitido los sin economía 👋🏼....`}
...
//_antispam.js
const smsNoSpam = () => { return `No hagas spam mierda , no podrás usar el bot en los siguientes monitos`}
